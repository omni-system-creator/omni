    ...
    <a-form :model="formState" :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
      <a-form-item label="供应商名称" required>
        <a-input-group compact>
          <a-input v-model:value="formState.partnerName" placeholder="请输入供应商名称" style="width: calc(100% - 32px)" :disabled="disabled" />
          <a-button v-if="hasSupplierPerm && !disabled" type="primary" @click="openSupplierSelector">
      ...
      <a-form-item label="应付金额" required>
...
import SupplierSelectorModal from '@/components/Selectors/SupplierSelectorModal.vue';
...
const modalTitle = computed(() => {
  if (props.disabled) return '应付单详情';
  return isEdit.value ? '编辑应付单' : '新增应付单';
});

// Permission check
const hasSupplierPerm = computed(() => permissionStore.hasMenu('ProcurementSupplier'));
...
const handleSupplierSelect = (supplier: any) => {
  formState.partnerName = supplier.name;
  formState.partnerId = supplier.id;
};
...
onMounted(async () => {
  if (subjects.value.length === 0) {
    subjects.value = await getSubjects('expense');
  }
});
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value && props.editData) {
      ...
      formData.append('type', 'AP'); // Fixed type for AP modal
      await createApAr(formData);
      message.success('创建应付单成功');
    }
    emit('success');
    visible.value = false;
  } finally {
