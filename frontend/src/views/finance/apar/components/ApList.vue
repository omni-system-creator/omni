<template>
  <div class="ap-list">
    ...
    <!-- Create Modal -->
    <ApCreateModal v-model:visible="createVisible" :edit-data="editData" :disabled="isReadOnly" @success="fetchData" />
...
import ApCreateModal from './ApCreateModal.vue';
...
const createVisible = ref(false);
  ...
  { title: '供应商', dataIndex: 'partnerName', key: 'partnerName' },
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await getApArList({ type: 'AP', partner: queryParam.supplier, status: queryParam.status || undefined });
    data.value = res || [];
...
const handleDelete = (record: FinanceApAr) => {
  Modal.confirm({
    title: '确认删除?',
    content: `确定要删除应付单 ${record.billNo} 吗？`,
    onOk: async () => {
      try {
        const res = await deleteApAr(record.id);
        if (res) {
