...
watchEffect(() => {
  ...
  if (props.assigneeType === 'choice') {
    ...
    if (props.users.length > 0) {
      ...
      const selectedUsers = userList.filter(item => props.users.includes(item.id));
      content.value = selectedUsers.map(user => user.chartConfig.title).join('、');
    } else {
      content.value = '未指定控件'
    }
    ...
    //     if(res.success){
    //       content.value = res.data.map((item) => item.name).join('、')
    //     }
    //   })
    // } else {
    //   content.value = '未指定角色'
