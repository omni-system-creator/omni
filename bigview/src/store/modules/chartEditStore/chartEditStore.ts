...
export const useChartEditStore = defineStore({
  ...
  actions: {
      ...
      if (targetIndex !== -1) {
        ...
        for (let i = 0; i < length; i++) {
          if (this.getComponentList[i].isGroup) {
            const groupList = (this.getComponentList[i] as CreateComponentGroupType).groupList
            for (let j = 0; j < groupList.length; j++) {
                if (groupList[j].id === targetId) {
                  return 100000 * (i + 1) + j
                }
            }
          }
    ...
    addComponentList(
      ...
      if (componentInstance instanceof Array) {
        componentInstance.forEach(item => {
          this.addComponentList(item, isHead, isHistory, groupIndex)
        })
        return
      }
      if (isHistory) {
      ...
      if (groupIndex !== undefined) {
        const groupList = this.getComponentList[groupIndex].groupList
        if(groupList) {
          list = groupList
        }
      }
      if (isHead) {
        list.unshift(componentInstance)
        return
      }
      list.push(componentInstance)
    },
    getComponentByIndex(index: number): CreateComponentType | CreateComponentGroupType {
      if(index>100000) {
        const groupIndex = Math.floor(index/100000)-1
        const groupList = this.getComponentList[groupIndex].groupList
        if(groupList) {
          const sindex = index%100000
          return groupList[sindex]
        }
      }
      else {
        return this.getComponentList[index]
      }
    ...
    removeComponentList(id?: string | string[], isHistory = true): void {
      try {
        ...
        idArr.forEach(ids => {
          ...
          if (index !== -1) {
            const component = this.getComponentByIndex(index)
            if(component!=null) {
    ...
    resetComponentPosition(item: CreateComponentType | CreateComponentGroupType, isForward: boolean): void {
      ...
      if (index > -1) {
        const componentInstance = this.getComponentByIndex(index)
    ...
    setBothEnds(isEnd = false, isHistory = true): void {
      try {
        ...
        loadingStart()
        let length = this.getComponentList.length
        if (length < 2) {
          loadingFinish()
          return
        }

        let index = this.fetchTargetIndex()
        const targetData = this.getComponentByIndex(index)
        let groupIndex : number | undefined = undefined;
        if(index>100000) {
          groupIndex = Math.floor(index/100000)-1
          const groupList = this.getComponentList[groupIndex].groupList
          if (!groupList) {
            loadingFinish()
            return
        ...
        if (index !== -1) {
          ...
          // 插入两端
          this.addComponentList(targetData, isEnd, false, groupIndex)
          ...
          list.splice(isEnd ? index + 1 : index, 1)
          loadingFinish()
          return
        }
      } catch (value) {
    ...
    wrap(isDown = false, isHistory = true) {
      try {
        ...
        let list = this.getComponentList
        let groupIndex = 0;
        let index: number = this.fetchTargetIndex()
        if (index !== -1) {
          let length = this.getComponentList.length
          if(index>100000) {
            groupIndex = Math.floor(index/100000)-1
          ...
          this.updateComponentList((groupIndex>0?(groupIndex+1) * 100000:0) + index, swapItem)
          this.updateComponentList((groupIndex>0?(groupIndex+1) * 100000:0) + swapIndex, targetItem)
          loadingFinish()
          return
        }
      } catch (value) {
      ...
      try {
        ...
        if (index !== -1) {
          const copyData: RecordChartType = {
            charts: this.getComponentByIndex(index),
    ...
    setMove(keyboardValue: MenuEnum) {
      ...
      if (index === -1) return
      const attr = this.getComponentByIndex(index).attr
    ...
    setGroup(id?: string | string[], isHistory = true) {
      try {
        ...
        selectIds.forEach((id: string) => {
          ...
          if (targetIndex !== -1 && this.getComponentByIndex(targetIndex).isGroup) {
            this.setUnGroup(
              [id],
    ...
    setUnGroup(ids?: string[], callBack?: (e: CreateComponentType[]) => void, isHistory = true) {
      try {
        ...
        const unGroup = (targetIndex: number) => {
          const targetGroup = this.getComponentByIndex(targetIndex) as CreateComponentGroupType
    ...
    setLock(status: boolean = true, isHistory: boolean = true) {
      try {
        ...
        if (index !== -1) {
          // 更新状态
          const targetItem = this.getComponentByIndex(index)
