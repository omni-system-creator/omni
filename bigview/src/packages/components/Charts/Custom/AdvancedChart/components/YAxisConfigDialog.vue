<template>
  <n-modal v-model:show="showModal" preset="dialog" title="Y轴配置" style="width: 650px;">
    <div class="yaxis-config-dialog">
      <!-- 基本配置 -->
      <setting-item-box name="基本配置">
        <setting-item name="轴名称">
          <n-input 
            ...
            size="small" 
            :placeholder="boundSeriesField ? `${boundSeriesField}（字段名）` : 'Y轴名称'" 
          />
        </setting-item>
        ...
        <setting-item name="名称旋转">
          <n-input-number
            v-model:value="axisData.nameRotate"
            size="small"
            :min="-90"
            :max="90"
            :step="1"
      ...
      <!-- 数值配置 -->
      <setting-item-box name="数值配置" v-if="axisData.type === 'value'">
        <setting-item name="最小值">
          <n-input 
            v-model:value="axisData.min" 
            size="small" 
        ...
        <setting-item name="分割段数">
          <n-input-number 
            v-model:value="axisData.splitNumber" 
            size="small" 
            :min="2"
            placeholder="分割段数"
          />
      ...
      <setting-item-box name="样式配置" :alone="true">
        <div style="padding-right: 10px;">
          <el-row :gutter="10">
            <el-col :span="6">
              <setting-item name="轴线颜色" v-if="axisData.axisLine.show" style="min-width: auto;">
                <n-color-picker v-model:value="axisData.axisLine.lineStyle.color" size="small" />
              </setting-item>
            </el-col>
            <el-col :span="6">
              <setting-item name="刻度颜色" v-if="axisData.axisTick.show" style="min-width: auto;">
                <n-color-picker v-model:value="axisData.axisTick.lineStyle.color" size="small" />
              </setting-item>
            </el-col>
            <el-col :span="6">
              <setting-item name="标签颜色" v-if="axisData.axisLabel.show" style="min-width: auto;">
                <n-color-picker v-model:value="axisData.axisLabel.color" size="small" />
              </setting-item>
            </el-col>
            <el-col :span="6">
              <setting-item name="网格线颜色" v-if="axisData.splitLine.show" style="min-width: auto;">
...
// 计算绑定到当前Y轴的系列数据字段名
const boundSeriesField = computed(() => {
  if (!props.containerData?.series || !Array.isArray(props.containerData.series)) {
    return ''
  }
  
  // 查找绑定到当前Y轴的系列
  const boundSeries = props.containerData.series.find((series: any) => 
    series.yAxisIndex === props.axisData?.yAxisIndex
  )
