...
const handleMapAdvChart = (val, obj) => {
  // components.id_1q60gtmomcqo00.props.chartConfig.request.requestSQLContent
  ...
  TASK_ID = obj.key
  const aux = obj.aux
  const fileds = obj.fileds
  DISTANCE = fileds[aux.x]?.label || 'Distance'
  LNG = fileds[aux.lon]?.label || 'longitude'
  LAT = fileds[aux.lat]?.label || 'latitude'
...
// proect/getMapData?x=Distance&lng=log&lat=lat&size=500&task_id=xx&y=Speed
function addNewLayerEx(deckOverlay, param_name, size, task_id, x, lon, lat, elevationScale, radius) {
  let baseUrl = location.protocol + '//' + location.hostname + (location.port ? ':' + location.port : '') + '/'
  const url = `goview/project/getMapData?y=${param_name}&size=${size}&task_id=${task_id}&x=${x}&lng=${lon}&lat=${lat}`
  ...
  layer = new HexagonLayer({
    ...
    // getColorWeight: d => (d.VALUES<1.5 ? 0:(d.VALUES<2? 1 : (d.VALUES<2.5? 2 : (d.VALUES<2.75 ? 3 : (d.VALUES<3? 4 : 5))))),
    // getColorWeight: d => (d.VALUES<1.5 ? 0:(d.VALUES<2 ? 2 : (d.VALUES<2.5? 4 : 5))),
    getColorWeight: d => (d.VALUES < 1.5 ? 0 : d.VALUES < 2 ? 1 : d.VALUES < 2.5 ? 2 : 3),
    ...
    radius: radius,
    colorRange: color_range_donglixue,
    pickable: true,
    material: {
      ambient: 0.8,
      shininess: 1,
  ...
  if (deckOverlay == null) {
    const deckOverlay = new DeckOverlay({
      ...
      getTooltip: ({ object }) =>
        // object &&
        // `${param_name}: ${object.elevationValue}\n距离: ${object.points[0].source.DISTANCE} 米\n经纬度: ${
        //   Math.round(object.position[0] * 100000) / 100000
        // },${Math.round(object.position[1] * 100000) / 100000}`
        object && `${param_name}: ${object.elevationValue}`
    })
    return deckOverlay
  } else {
    deckOverlay.setProps({
...
function addNewLayer(deckOverlay, param_name, size, elevationScale, radius) {
  const DONGLIXUE = param_name == 'pingwen_dong_heng' || param_name == 'pingwen_dong_chui'
  ...
  if (deckOverlay == null) {
    const deckOverlay = new DeckOverlay({
      ...
      getTooltip: ({ object }) =>
        // object &&
        // `${label_cn}: ${object.elevationValue}\n距离: ${object.points[0].source.DISTANCE} 米\n经纬度: ${
        //   Math.round(object.position[0] * 100000) / 100000
        // },${Math.round(object.position[1] * 100000) / 100000}`
        object && `${param_name}: ${object.elevationValue}`
    })
...
const initMap = async (newData: any) => {
  ...
  map.on('load', async () => {
    ...
    if (propRequest.value && propRequest.value.key) {
      hotLayer = addNewLayerEx(
        null,
        'Speed',
        zoomArr[nowId].hot_size,
        propRequest.value.key,
        'Distance',
        'longitude',
        'latitude',
        zoomArr[nowId].elevationScale,
...
function updateMapRequest(option) {
  propRequest.value = option
  console.log(propRequest.value, 'xxxxxxxxxxxxxxxx')
}
...
watch(
  () => props.chartConfig.request.requestSQLContent,
  option => {
    try {
      updateMapRequest(option)
    } catch (error) {
      console.log(error)
    }
  },
