 ...
 */
 export function generateBaseFunc(fnStr: string) {
  try {
    return new Function(`
      return (
        async function(components, http, mouseEvent){
          ${fnStr}
        }
      )`)().bind(undefined,components, http) // 兼容onchange
  } catch (error) {
    console.error(error)
  }
}
 ...
 */
export function generateFunc(fnStr: string, e: any) {
  try {
    // npmPkgs 便于拷贝 echarts 示例时设置option 的formatter等相关内容
    Function(`
      "use strict";
      return (
        async function(e, components, node_modules, http){
          const {${Object.keys(npmPkgs).join()}} = node_modules;
          ${fnStr}
        }
      )`)().bind(e?.component)(e, components, npmPkgs, http)
  } catch (error) {
    console.error(error)
  }
}
