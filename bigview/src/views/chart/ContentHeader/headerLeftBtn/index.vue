...
import { useChartHistoryStore, useChartHistoryList } from '@/store/modules/chartHistoryStore/chartHistoryStore'
...
// 返回首页
const goHomeHandle = () => {
  const isDataSaved = computed(() => chartHistoryStore.getBackStack?.length === chartHistoryStore.useChartHistoryList?.length);
  console.log('isDataSaved', isDataSaved.value)
  if (isDataSaved.value || chartHistoryStore.getBackStack?.length ===1) {
    // 数据已经保存，直接返回首页
    console.log('数据已经保存，直接返回首页')
    goHome();
    useRemoveKeyboard();
  } else {
    // 数据未保存，显示确认对话框
    console.log(chartHistoryStore.getBackStack.length) 
    goDialog({
