...
export const useMouseHandle = () => {
  ...
  const mousedownHandle = (e: MouseEvent, item: CreateComponentType | CreateComponentGroupType) => {
    ...
    chartEditStore.getTargetChart.selectId.forEach(id => {
      ...
      if (index !== -1) {
        const { x, y, w, h } = toRaw(chartEditStore.getComponentByIndex(index)).attr
    ...
    chartEditStore.getTargetChart.selectId.forEach(id => {
      ...
      const index = chartEditStore.fetchTargetIndex(id)
      // 拿到初始位置数据
      prevComponentInstance.push(cloneDeep(chartEditStore.getComponentByIndex(index)))
    })
    ...
    const mousemove = throttle((moveEvent: MouseEvent) => {
      ...
      chartEditStore.getTargetChart.selectId.forEach(id => {
        ...
        const componentInstance = chartEditStore.getComponentByIndex(index)
      ...
      try {
        ...
        if (prevComponentInstance.length) {
          chartEditStore.getTargetChart.selectId.forEach(id => {
            if (!targetMap.has(id)) return
            const index = chartEditStore.fetchTargetIndex(id)
            const curComponentInstance = chartEditStore.getComponentByIndex(index)
            // 找到记录的所选组件
