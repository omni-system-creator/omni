...
window.addEventListener('beforeunload', function(event) {
    const isBackStack =chartHistoryStore.getBackStack?.length ==chartHistoryStore.useChartHistoryList?.length
    console.log(chartHistoryStore.getBackStack?.length,chartHistoryStore.useChartHistoryList?.length,'1111111111111111')
    // 询问用户是否保存数据
    if (!isBackStack){
      const confirmationMessage = '您确定要离开此页面吗？您所做的更改将丢失。';
      event.preventDefault(); // 或者 event.returnValue = confirmationMessage;
