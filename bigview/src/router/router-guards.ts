...
export function createRouterGuards(router: Router) {
  ...
  router.beforeEach(async (to, from, next) => {
    ...
    if (to.query.token) {
      // 第三方登录token
      useSystemStore().setItem(SystemStoreEnum.TOKEN,to.query.token as string);
      await useSystemStore().afterLoginAction(true);
    }
    // if (!useSystemStore().getToken){
    await useSystemStore().decryptToken()
    // }
